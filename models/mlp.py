...
class MLP(BaseModel):
    def __init__(self, input_size, hidden_size, output_size,
                 n_layers, horizon, dropout = 0.1, activation='relu', exog_size=0, n_nodes=None, use_node_embeddings=False):
        super().__init__()
        ...
        self.fc = subMLP(hidden_size * 12, output_size * horizon, hidden_size=hidden_size, dropout=dropout, n_layers=n_layers)
    ...
    def forward(self, x, u=None, edge_index=None, edge_weight=None, enable_mask=None):
        ...
        x = einops.rearrange(x, 'b t n h -> (b n) (t h)')

        # Fully connected layer
        x = self.fc(x)

        # rearrange output to match the expected shape
        x = einops.rearrange(x, '(b n) h -> b 1 n h', b=batch_size, n=num_nodes)
