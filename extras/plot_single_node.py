...
def get_dl_predictions(predictor, data_module):
    ...
    # Extract first feature for single node (shape becomes B, T, 1)
    y = y[:, :, :, 0]
    y_hat = y_hat[:, :, :, 0]
...
def get_arimax_predictions(predictor, data_loader):
    ...
    # Reshape if needed - ensure shape is (B, T, 1)
    if len(y_hat.shape) == 2:
        y_hat = y_hat[:, :, np.newaxis]  # Add node dimension
...
def plot_predictions_common(prediction_data, run_dir, log_metrics=None, delay=14, wandb_run=None):
    """
    Common plotting function for single node predictions.
    ...
    for h in horizons:
        ...
        plt.title(f'Single Node - Horizon {h} - Delay {delay} - Factor: {factor:.2f}')
        plt.tight_layout()
        plt.savefig(plot_dir / f'single_node_horizon_{h}.png')
        print(f'Saved plot for Single Node - Horizon {h}')
        plt.close()
        ...
        plt.title(f'Single Node - Horizon {h} - Delay {delay} - Factor: {factor:.2f} (Full Range)')
        plt.tight_layout()
        plt.savefig(plot_dir / f'single_node_horizon_{h}_full.png')
        print(f'Saved full plot for Single Node - Horizon {h}')
        plt.close()

    if prediction_data["flag_increment"] is not None:
        plot_binary_flags(prediction_data, run_dir, delay=delay)
...
def plot_predictions_test_single(predictor, data_module, run_dir, model_type='dl', arimax_test_data=None, 
                         log_metrics=None, delay=14, wandb_run=None):
    """
...
def plot_binary_flags(prediction_data, run_dir, delay=14):
    """
    ...
    # Plot binary flags for each horizon
    for h in horizons:
        ...
        ax1.legend()
        ax1.set_title(f'Real Signal - Single Node, Horizon {h}')
        ...
        # Save plot
        plt.savefig(plot_dir / f'single_node_horizon{h}_flags.png', dpi=300, bbox_inches='tight')
        print(f'Saved binary flag plot for Single Node - Horizon {h}')
        ...
        ax1.grid(True, alpha=0.3)
        ax1.legend()
        ax1.set_title(f'Full Range Real Signal - Single Node, Horizon {h}')
        ...
        plt.savefig(plot_dir / f'single_node_horizon{h}_flags_full.png', dpi=300, bbox_inches='tight')
        print(f'Saved full binary flag plot for Single Node - Horizon {h}')
